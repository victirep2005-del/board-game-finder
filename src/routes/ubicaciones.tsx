...
const locationsQueryOptions = queryOptions({
  queryKey: ["locations"],
  queryFn: async () => {
    const [rooms, shelves] = await Promise.all([
      listRooms(),
      listShelves(),
    ]);
  ...
  head: () => ({
    meta: [
      ...
      },
      {
        name: "description",
        content:
          "Gestiona los cuartos y estantes del local.",
      },
      {
        property: "og:title",
        content: "Ubicaciones — BoardGameFinder",
      },
      {
...
function LocationsPage() {
  ...
  const handleDeleteRoom = async (
    ...
    try {
      ...
      toast.error(
        "No se pudo eliminar el cuarto. Asegúrate de que no tenga estantes."
      );
    }
  };
  ...
  const handleUpdateShelf = async (
    ...
    try {
      ...
      toast.error(
        "No se pudo actualizar el estante"
      );
    }
  };
  ...
  const handleDeleteShelf = async (
    ...
    try {
      ...
      toast.error(
        "No se pudo eliminar el estante. Asegúrate de que no tenga juegos."
      );
    }
  };
        ...
        >
          {locations.rooms.map((room) => {
            ...
            return (
              <div
                key={room.id}
                className="rounded-xl border border-white/20 bg-black/30 p-4 shadow-md backdrop-blur-sm"
              >
                {/* Cuarto */}
                ...
                <div className="mt-3 space-y-2">
                  {roomShelves.length === 0 ? (
                    ...
                    roomShelves.map((shelf) => (
                      <div
                        key={shelf.id}
                        className="flex items-center justify-between gap-2 rounded-md border border-white/10 bg-black/25 px-3 py-2"
                      >
                        <div className="flex items-center gap-2">
                          <Rows3 className="h-4 w-4 text-white/70" />
                  ...
                  <LocationDialog
                    title={`Nuevo estante en ${room.name}`}
                    triggerLabel="Añadir estante"
                    variant="ghost"
                    size="sm"
                  >
                    <ShelfForm
...
function LocationSection({
  ...
  return (
    <section>
      <div className="mb-3 flex items-center justify-between rounded-xl border border-white/10 bg-black/20 p-3 backdrop-blur-sm">
        <div className="flex items-center gap-2">
          {icon}
