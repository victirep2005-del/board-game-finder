...
import { boardGameFormSchema, type BoardGameFormData, type BoardGameWithLocation, type ShelfWithRoom, difficultyLabels, type Difficulty, type GameCategory } from "@/lib/schemas";
import { GameImageUpload } from "@/components/game-image-upload";

const categoryOptions: GameCategory[] = ["Cartas", "Cooperativo", "Dados", "Estrategia", "Estrategia Abstracta", "Estrategia Ligera", "Fiesta", "Roles Ocultos", "Temático"];
...
export function GameForm({ initialData, shelves, onSubmit, isSubmitting, submitLabel = "Guardar" }: GameFormProps) {
  const { register, handleSubmit, setValue, watch, formState: { errors } } = useForm<BoardGameFormData>({
    ...
    defaultValues: initialData
      ? { name: initialData.name, category: initialData.category ?? null, shelfId: initialData.shelfId, minPlayers: initialData.minPlayers, maxPlayers: initialData.maxPlayers, durationMinutes: initialData.durationMinutes, minAge: initialData.minAge, difficulty: initialData.difficulty, notes: initialData.notes ?? "", imageUrl: initialData.imageUrl ?? null }
      : { name: "", category: null, shelfId: "", minPlayers: null, maxPlayers: null, durationMinutes: null, minAge: null, difficulty: "casual", notes: "", imageUrl: null },
  });
  const selectedShelfId = watch("shelfId"); const category = watch("category"); const difficulty = watch("difficulty"); const imageUrl = watch("imageUrl");
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div className="space-y-2"><Label htmlFor="category" className="text-foreground">Categoría</Label><Select value={category ?? ""} onValueChange={(value) => setValue("category", value as GameCategory, { shouldValidate: true, shouldDirty: true })}><SelectTrigger id="category" className={selectClass}><SelectValue placeholder="Selecciona una categoría" /></SelectTrigger><SelectContent className="border-white/20 bg-slate-900 text-foreground">{categoryOptions.map((value) => <SelectItem key={value} value={value}>{value}</SelectItem>)}</SelectContent></Select>{errors.category && <p className="text-sm text-destructive">{String(errors.category.message ?? "")}</p>}</div>
